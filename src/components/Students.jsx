...
const Students = () => {
  ...
  const updateStudentList = () => {
    ...
    const workspaceStudentIds = workspace.map((item) => item.student.id);
    ...
    const validStudents = students
      ...
      .map((item) => ({
        ...
        isInWorkspace: workspaceStudentIds.includes(item.student.id), // Check if student is in workspace
      }));
  ...
  return (
    ...
    >
      ...
      >
        <Text>
          {t("Total Students In")} <Text as="b">{t("History")}</Text> :{" "}
          {studentList.length}
        </Text>
        <HStack>
          <IconButton
